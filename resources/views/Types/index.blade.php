...
@section('content')
    ...
    <x-header>Vehicle Type</x-header>
    ...
    <div class="px-3 py-4 flex justify-center">
        <table class="w-full text-md bg-white shadow-md rounded mb-4">
            ...
            <tr class="border-b border-2 border-white rounded hover:bg-gray-400 bg-gray-500">
                ...
                <td class="p-3 px-5 flex justify-end">
                    <a href="{{ route('Type.create') }}"
                       class="mr-3 font-bold text-sm bg-green-500 hover:bg-green-700 text-white shadow-2xl py-1 px-4 rounded focus:outline-none focus:shadow-outline">
                        ADD</a>
                </td>
            </tr>
                ...
                <tr class="border-b hover:bg-gray-400 bg-gray-100">
                    ...
                    <td class="p-3 px-5 flex justify-end">
                        <a href="{{ route('Type.edit',$type['id']) }}"
                           class="mr-3 text-sm bg-blue-500 hover:bg-blue-700 text-white py-1 px-4 rounded focus:outline-none focus:shadow-outline">Edit</a>
                        <form action="{{ route('Type.destroy',$type['id']) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                    onclick="return confirm('Are You Sure Want to Delete {{ $type['vehicle_types_nm'] }}')"
            ...
            @empty
                <tr class="border-b hover:bg-gray-400 bg-gray-100">
                    <td colspan="3" class="p-3 text-center text-xl text-cool-gray-500 px-5">No Vehicle Types Have Been
                        Added
                    </td>
                </tr>
            @endforelse
